...
export const SUPPORTED_HARNESS_LANGUAGES = ['Chinese', 'English'] as const;
...
const CHINESE_PACK: HarnessLanguagePack = {
	language: 'Chinese',
	projectConstraintsTitle: 'Harness 项目约束',
	...
	statusBar: {
		idleText: '$(rocket) Harness Runner',
		idleTooltip: 'Harness Runner：点击显示命令菜单',
	...
	runtime: {
		...
		stalledTaskWarning: taskId => `HARNESS：任务 ${taskId} 在上一次中断运行后仍保持为“inprogress”。`,
		clearAndRetry: '清理并重试',
		cancel: '取消',
		projectConstraintsRequiredBeforeRun: 'HARNESS：执行前必须先初始化项目约束，请先运行“HARNESS: 初始化项目约束”。',
		policyBlockedBeforeStory: storyId => `HARNESS：机器策略门禁阻止了 ${storyId} 开始执行，请先根据输出面板中的缺失项完成处理。`,
		policyBlockedAfterStory: storyId => `HARNESS：机器策略门禁阻止了 ${storyId} 完成，请先处理输出面板中列出的缺失项或失败命令。`,
		allStoriesCompleted: 'HARNESS：所有用户故事均已完成！',
		pausedAfterLoops: count => `HARNESS 已在执行 ${count} 个步骤后暂停。运行“HARNESS: 开始执行”即可继续。`,
		notRunning: 'HARNESS 当前未运行。',
		stopped: 'HARNESS 已停止。',
		designContextRequiredBeforeStory: storyId => `HARNESS：执行 ${storyId} 前必须先补充界面设计描述。请先运行“HARNESS: 界面设计描述”。`,
	},
	status: {
		...
		summary: (completed, total, nextId) => `HARNESS：已完成 ${completed}/${total} 个故事。下一个：${nextId || '全部完成！'}`,
		none: '无',
		allDone: '全部完成！',
		yes: '是',
		no: '否',
	...
	approval: {
		...
		updated: (storyId, status) => `HARNESS：${storyId} 的审批结果已更新，当前状态：${status}。`,
		openEvidence: '打开证据包',
		openFlow: '打开审批流',
		required: (storyId, status) => `HARNESS：${storyId} 当前为“${status}”，需要人工审批。`,
		historyHeading: '审批记录：',
		noHistory: '暂无审批记录。',
	...
	policyConfig: {
		...
		approvalModePlaceholder: '选择审批模式的保存值；Workspace 优先，未覆盖时回退到 User 全局设置',
		reviewerLoopPlaceholder: '选择 Reviewer 配置的保存值；Workspace 优先，未覆盖时回退到 User 全局设置',
		...
		reviewerPassingScorePrompt: '输入保存到所选作用域的通过分数（1-100）',
		reviewerPassingScorePlaceholder: '例如 85',
		reviewerPassingScoreValidation: '请输入 1 到 100 之间的整数分数。',
		autoRefactorRoundsPrompt: '输入保存到所选作用域的自动修复轮数（0 或更大）',
		autoRefactorRoundsPlaceholder: '例如 2',
		autoRefactorRoundsValidation: '请输入大于等于 0 的整数轮数。',
		saved: 'HARNESS：执行检查、审批提示模式和 Reviewer 配置已更新；若保存到 User，全局值已写入并清除了当前工作区覆盖。',
		openSettings: '打开设置',
		ruleLabels: {
		...
		approvalModes: {
			default: { label: 'default：弹出审批提示', description: '工作区有覆盖时优先使用工作区；否则回退到 User 全局设置' },
			bypass: { label: 'bypass：直接进入审批流', description: '工作区有覆盖时优先使用工作区；否则回退到 User 全局设置' },
			autopilot: { label: 'autopilot：仅落盘并挂到状态栏', description: '工作区有覆盖时优先使用工作区；否则回退到 User 全局设置' },
		},
	},
	initProjectConstraints: {
		...
		languageChanged: 'HARNESS 语言已切换。若要让项目约束中的语言相关规则同步更新，请重新初始化项目约束。',
	...
	designContext: {
		noStories: '未找到可用的用户故事，请先准备 prd.json。',
			...
			noReusableDrafts: 'HARNESS：当前还没有可复用的界面设计描述，请先创建项目级、页面级或模块级描述。',
			noPendingStories: 'HARNESS：当前没有可匹配的未完成用户故事。',
		saved: (storyId, hasWarnings) => hasWarnings ? `HARNESS：已为 ${storyId} 保存界面设计描述，但存在警告。` : `HARNESS：已为 ${storyId} 保存界面设计描述。`,
		open: '打开界面设计描述',
	...
	taskMemoryRecall: {
		noRelatedTaskMemories: storyId => `HARNESS：未找到与 ${storyId} 相关的任务记忆。`,
	...
	appendStories: {
		...
		requestTitle: 'HARNESS 追加用户故事：描述新增需求',
	...
	quickStart: {
		...
		provideSuccess: 'HARNESS：prd.json 已准备完成，现在可以执行“HARNESS: 开始执行”。',
		goalTitle: 'HARNESS 生成 PRD：描述你的目标',
...
const ENGLISH_PACK: HarnessLanguagePack = {
	language: 'English',
	projectConstraintsTitle: 'Harness Project Constraints',
	...
	statusBar: {
		idleText: '$(rocket) Harness Runner',
		idleTooltip: 'Harness Runner: click to show the command menu',
		runningText: '$(sync~spin) Harness Runner',
		runningTooltip: 'Harness Runner: tasks are running, click to open the menu',
		pendingApprovalsText: count => `$(pass-filled) Harness Runner ${count}`,
		pendingApprovalsTooltip: count => `Harness Runner: ${count} stories are waiting for approval. Click to open the menu or run "HARNESS: Review Approval".`,
	},
	runtime: {
		alreadyRunning: 'HARNESS is already running.',
		prdNotFoundRoot: 'prd.json not found in the workspace root.',
		stalledTaskWarning: taskId => `HARNESS: Task ${taskId} was left as "inprogress" from a previous interrupted run.`,
		clearAndRetry: 'Clear and Retry',
		cancel: 'Cancel',
		projectConstraintsRequiredBeforeRun: 'HARNESS: Project constraints are required before execution. Run "HARNESS: Initialize Project Constraints" first.',
		policyBlockedBeforeStory: storyId => `HARNESS: Machine policy gates blocked ${storyId} before execution. Review the missing items in the output panel first.`,
		policyBlockedAfterStory: storyId => `HARNESS: Machine policy gates blocked completion for ${storyId}. Fix the listed missing artifacts or failing commands first.`,
		allStoriesCompleted: 'HARNESS: All user stories completed!',
		pausedAfterLoops: count => `HARNESS paused after ${count} steps. Run "HARNESS: Start" to resume.`,
		notRunning: 'HARNESS is not running.',
		stopped: 'HARNESS stopped.',
		designContextRequiredBeforeStory: storyId => `HARNESS: UI design notes are required before executing ${storyId}. Run "HARNESS: UI Design Notes" first.`,
	},
	status: {
		...
		summary: (completed, total, nextId) => `HARNESS: ${completed}/${total} stories done. Next: ${nextId || 'Complete!'}`,
		none: 'None',
		allDone: 'All done!',
		yes: 'Yes',
		no: 'No',
	...
	approval: {
		...
		updated: (storyId, status) => `HARNESS: Updated approval for ${storyId}. Current status: ${status}.`,
		openEvidence: 'Open Evidence',
		openFlow: 'Open Approval Flow',
		required: (storyId, status) => `HARNESS: ${storyId} is currently ${status} and requires manual approval.`,
	...
	policyConfig: {
		title: 'HARNESS: Configure Run Checks',
		...
		approvalModePlaceholder: 'Choose the approval mode to save; workspace overrides win, otherwise user settings apply',
		reviewerLoopPlaceholder: 'Choose the Reviewer settings to save; workspace overrides win, otherwise user settings apply',
		...
		reviewerPassingScorePrompt: 'Enter the passing score to save into the selected scope (1-100)',
		reviewerPassingScorePlaceholder: 'For example: 85',
		reviewerPassingScoreValidation: 'Enter an integer score between 1 and 100.',
		autoRefactorRoundsPrompt: 'Enter how many auto-refactor rounds to save into the selected scope (0 or more)',
		autoRefactorRoundsPlaceholder: 'For example: 2',
		autoRefactorRoundsValidation: 'Enter an integer greater than or equal to 0.',
		saved: 'HARNESS: Run checks, approval mode, and Reviewer settings were updated; saving to User also clears current workspace overrides.',
		...
		approvalModes: {
			default: { label: 'default: show an approval notification', description: 'Workspace overrides win; otherwise the saved user setting applies' },
			bypass: { label: 'bypass: open the approval flow directly', description: 'Workspace overrides win; otherwise the saved user setting applies' },
			autopilot: { label: 'autopilot: persist only and surface via status bar', description: 'Workspace overrides win; otherwise the saved user setting applies' },
		},
	},
	initProjectConstraints: {
		...
		languageChanged: 'HARNESS language changed. Reinitialize project constraints if you want language-dependent rules to refresh.',
	...
	designContext: {
		noStories: 'No user stories were found. Prepare prd.json first.',
			...
			noReusableDrafts: 'HARNESS: There are no reusable UI design notes yet. Create project-, screen-, or module-level notes first.',
			noPendingStories: 'HARNESS: There are no unfinished stories available for matching.',
		saved: (storyId, hasWarnings) => hasWarnings ? `HARNESS: Saved UI design notes for ${storyId}, but warnings were found.` : `HARNESS: Saved UI design notes for ${storyId}.`,
	...
	taskMemoryRecall: {
		noRelatedTaskMemories: storyId => `HARNESS: No related task memories found for ${storyId}.`,
	...
	appendStories: {
		...
		requestTitle: 'HARNESS Append User Stories: describe the new scope',
	...
	quickStart: {
		...
		provideSuccess: 'HARNESS: prd.json is ready. You can now run "HARNESS: Start".',
		goalTitle: 'HARNESS Generate PRD: describe your goal',
...
const LANGUAGE_PACKS: Record<SupportedHarnessLanguage, HarnessLanguagePack> = {
	Chinese: CHINESE_PACK,
	English: ENGLISH_PACK,
};

export function normalizeHarnessLanguage(value: string | undefined): SupportedHarnessLanguage {
	return value === 'English' ? 'English' : 'Chinese';
}

export function getHarnessLanguagePack(value: string | undefined): HarnessLanguagePack {
	return LANGUAGE_PACKS[normalizeHarnessLanguage(value)];
}

export function getLocalizedStoryStatus(status: StoryExecutionStatus | 'none', language: string | undefined): string {
	const normalizedLanguage = normalizeHarnessLanguage(language);
