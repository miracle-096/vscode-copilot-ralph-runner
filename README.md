...
RALPH（Run Autonomous Loops Per Handoff）是一个 VS Code 扩展，用来把 `prd.json` 里的用户故事转成面向 Cline 的上下文增强执行循环。每次执行都会注入项目约束、设计上下文、既往任务记忆、源码上下文、执行检查点，以及更严格的完成契约。

如果你希望智能体在持续执行实现类故事时，仍然遵守仓库规则、UI 设计意图和已有决策，RALPH 就是为这个场景准备的。

Harness Runner 会直接把执行提示词交给新的 Cline 任务；如果 Cline API 暂时不可用，则退回到聚焦 Cline 侧栏并复制提示词到剪贴板的兜底流程。
...
Harness Runner 的状态栏入口和快捷键 Alt+R 会打开分层命令菜单。当前一级菜单分为四组：
...
两个二级菜单都保留“返回上一级”导航，因此一级菜单只负责分组，具体动作在分组内完成。菜单排序界面使用拖拽卡片调整顺序，只有点击保存后才会写入工作区 `.vscode/settings.json`，取消或直接关闭不会落盘。

这样做的目标是把执行前准备动作和真正的执行/审批动作拆开，避免一级菜单堆叠过多入口，同时保持原有命令 id 和命令路由不变。
...
- `completionRules`：Cline 写出完成信号后、RALPH 接受故事完成前执行
...
RALPH 不再只依赖 `completed` 文件信号来认定故事完成。Cline 在结束前必须写出：
...
如果 Cline 没有写出合法工件，RALPH 会合成一个可恢复的 fallback 工件，并继续推进，而不是因为上下文损坏直接卡死。
...
执行 `HARNESS: 显示菜单` 后，一级菜单固定分为五类：`规划与入门`、`Harness-runner 指南`、`Harness 约束设置`、`执行与审批`、`设置`。

- `规划与入门` 下继续承载 `生成 PRD`、`追加用户故事`
...
| `harness-runner.rootMenuOrder` | `['planning', 'constraints', 'execution', 'settings']` | string[] | 自定义一级菜单顺序，缺失或重复值会自动回退到默认顺序；通过“自定义菜单排序”保存时会写入工作区的 `.vscode/settings.json` |
...
- 如果 Cline 没有写出合法的任务记忆、检查点或证据包，RALPH 会合成 fallback 工件而不是直接卡死
...
- 如果无法直接注入到 Cline，RALPH 会回退为复制提示词到剪贴板并聚焦 Cline 侧栏
