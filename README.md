...
| 路径 | 作用 |
| --- | --- |
| `.ralph/story-status.json` | 共享的故事状态表，同时承载故事级持久状态与运行期完成信号键 |
...
| `.ralph/agent-map/overview.json` | 面向智能体的仓库总览页，聚合项目概览、模块职责、规则入口、执行 runbook 与文档索引 |
| `.ralph/agent-map/knowledge-catalog.json` | 面向智能体的知识目录页，按主题列出关键文档、工件与显式知识缺口 |
...
| `.ralph/run-logs/US-xxx-*.run-log.txt` | 单次故事执行的文本运行日志，只保留 output 中筛选后的关键信息与最终摘要 |
...
`progress.txt` 继续承担面向人的高层进度跟踪；更细的运行级信号会增量写入 `.ralph/run-logs/`。当前 run log 不再写结构化 JSON，而是以纯文本方式记录 output 中筛选出的关键信息，主动跳过 Copilot 轮询噪音，只保留适合人工快速阅读的关键摘要。
...
执行 `RALPH: 生成 Agent Map` 后，RALPH 会把轻量级导航工件写入 `.ralph/agent-map/`：
...
从当前版本开始，RALPH 在执行故事和 `@ralph /ralph-spec` 时会自动读取 `overview.json` 与 `knowledge-catalog.json`，结合故事描述、当前变更面和 README / runbook 文本，生成三类检查结果：
...
默认情况下，这些结果会作为 advisory 注入到故事提示词与 `/ralph-spec` 最终提示词里；如果你在 `policyGates` 里启用 `knowledge-check` 规则，也可以把 warning 级问题升级为完成前 gate。
...
如果工作区已经存在 Agent Map，`/ralph-spec` 还会附带知识目录过期、模块知识缺失、运行说明覆盖不足等提醒，帮助最终提示词显式要求补齐缺口，而不是只给出一条笼统的“文档不足”提示。
...
- 旧版任务锁文件 `.ralph/task-<id>-status` 如果仍存在，会在读取时迁移回 `.ralph/story-status.json`，之后不再作为主状态来源
